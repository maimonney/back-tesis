...
const buscarVuelosDeVuelta = async (req, res) => {
    ...
    console.log('Consulta de vuelta recibida:', { departure_id, arrival_id, outbound_date, return_date, departure_token });

    if (!departure_id || !arrival_id || !outbound_date || !return_date || !departure_token) {
        console.log('Faltan parámetros requeridos.');
        return res.status(400).json({
            error: 'Faltan parámetros requeridos: departure_id, arrival_id, outbound_date, return_date y departure_token.'
        });
    }
    ...
    try {
        ...
        const vuelosDeVuelta = [];

        if (response.data && response.data.best_flights && Array.isArray(response.data.best_flights)) {
            vuelosDeVuelta.push(...response.data.best_flights);
        }

        if (response.data && response.data.other_flights && Array.isArray(response.data.other_flights)) {
            vuelosDeVuelta.push(...response.data.other_flights);
        }

        if (vuelosDeVuelta.length > 0) {
            return res.json(vuelosDeVuelta);
        } else {
            return res.status(404).json({ error: 'No se encontraron vuelos de vuelta para los criterios solicitados.' });
        }

    } catch (error) {
        console.error('Error al buscar vuelos de vuelta en SerpAPI:', error);
        return res.status(500).json({
            error: 'Error al buscar vuelos de vuelta en SerpAPI.',
            message: error.message,
            details: error.response ? error.response.data : "Sin detalles adicionales"
        });
    }
